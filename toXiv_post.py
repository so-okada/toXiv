...
def main(switches, logfiles, captions, aliases, pt_mode):
    ...
    if not logfiles:
        ...
        if (
            crosslisting_mode[cat]
            or toot_replacement_mode[cat]
            or boost_replacement_mode[cat]
        ):
            ptext = (
                "No logfiles found. "
    ...
    threads = []
    for i, cat in enumerate(switches):
        if entries_dict[cat] and grouped_crosslists_mode[cat]:
            crosslisting_entries = entries_dict[cat].crosslists
            th = Thread(
                name=cat,
                target=grouped_crosslists,
                args=(
                    logfiles,
                    cat,
                    username_dict[cat],
            ...
            threads.append(th)
            print("start a grouped-crosslists thread of " + th.name)
            th.start()
            if i != len(switches) - 1:
    ...
    if pt_method == "toot":
        ...
        try:
            result = api.status_post(
                text, in_reply_to_id=tot_id_str, visibility=visibility
            )
            update_print(
                cat,
                aim,
...
def newentries(
    ...
    try:
        ...
        traceback.print_exc()
        if not check_log_dates(
            cat, username, "newsubmission_log", logfiles
        ) and not check_log_dates(cat, username, "newsubmission_summary_log", logfiles):
    ...
    if newsubmission_mode:
        ...
        if entries_dict[cat]:
            ...
            if not check_log_dates(
                cat, username, "newsubmission_log", logfiles
            ) and not check_log_dates(
                cat, username, "newsubmission_summary_log", logfiles
            ):
                newsubmissions(
                    logfiles,
                    cat,
...
def toot_replacement(
    ...
    time_now = datetime.utcnow().replace(microsecond=0)
    if pt_mode and any(
        edm.match(time_now, datetime.fromisoformat(t))
...
def grouped_crosslists(
    ...
    splitted_ptext = tXf.add_chunk_counter(
        grouped_crosslist_paper_chunks,
        "Crosslisted article(s) found for "
        + cat
        + ".  "
        + "https://arxiv.org/list/"
        + cat
